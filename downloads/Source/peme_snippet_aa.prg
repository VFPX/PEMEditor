...
Function Process
    ...
    lcText = StrExtract(lcParam1, '(', ')')
    ALines(laText, lcText, 15, ',')
    lcNewText = 'With This'
    lcNewText = lcNewText + CR + '.ControlSource = Thisform.oPrefs.l' + Chrtran(laText(1), ['"], '') 
    lcNewText = lcNewText + CR + '.Caption = ' + laText(3) 
    If Alen(laText) >= 4
	    lcNewText = lcNewText + CR + '.ToolTipText = ' + laText(4) 
    EndIf     
    lcNewText = lcNewText + CR + 'EndWith'
    Return lcNewText
EndFunc
