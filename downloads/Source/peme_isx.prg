...
Local laUsedAfter[1], laUsedBefore[1], lnI, lnUsed
Push Key Clear
If 0 = Aused(laUsedBefore)
	laUsedBefore[1] = '?'
Endif
...
For lnI = 1 To m.lnUsed
	If 0 = Ascan(m.laUsedBefore, m.laUsedAfter[m.lnI, 1], 1, -1, 1, 15)
...
Lparameters toEditBox, toThisform, toThis, tcProviders, tcInvocation, tlForceThor
	...
	*--------------------------------------------------------------------------------------
	...
	m.loISX.AddProperty("nEditSource",1)
	...
	*--------------------------------------------------------------------------------------
	Do While Len(m.loISX.cTextLeft) > 0
		m.lcChar = Right( m.loISX.cTextLeft, 1 )
		If IsAlpha(m.lcChar) or IsDigit(m.lcChar) or m.lcChar == "_" or m.lcChar == "." or m.lcChar == "@"
			m.loISX.cTextLeft = Left( m.loISX.cTextLeft, Len(m.loISX.cTextLeft)-1 )
...
Procedure CP_Tables
	...
	m.toISX.nCount = Alen(m.laList, 1)
